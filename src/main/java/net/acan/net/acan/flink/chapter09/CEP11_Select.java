...
public class CEP11_Select {
    public static void main(String[] args) {
        ...
        //4.从模式流中取出匹配的数据
      ps.flatSelect(new PatternFlatSelectFunction<WaterSensor, WaterSensor>() {
          @Override
          public void flatSelect(Map<String,
                  List<WaterSensor>> map,
                                 Collector<WaterSensor> collector) throws Exception {
              for (WaterSensor ws1 : map.get("s1")) {
                  collector.collect(ws1);
              }
              for (WaterSensor ws2 : map.get("s2")) {
                  collector.collect(ws2);
              }
          }
      })
              .print();
